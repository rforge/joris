 ...
 * @author Ralf
 */
public class REngineAssignIntegrationTest extends AbstractRIntegrationTest {
    ...
    public void testMatrix() {
        ...
        for (int row = 0;row < matrix.getRows(); row++) {
            for (int col = 0;col < matrix.getColumns(); col++) {
                Assert.assertEquals("coeff type is wrong @(row="+row+",col="+col+")", matrix.getValueAt(row, col).getType(), IREXP.XT_DOUBLE);
            }
        }
        Assert.assertEquals("coeff value is wrong @(row=0,col=0)",-17.579095, matrix.getValueAt(0, 0).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=1)",6.7584402, matrix.getValueAt(0, 1).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=2)",-2.601058, matrix.getValueAt(0, 2).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=3)",1.231882e-02, matrix.getValueAt(0, 3).asDouble(),  WrappedEngineIntegrationTest.EPS);

        Assert.assertEquals("coeff value is wrong @(row=1,col=0)",3.932409, matrix.getValueAt(1, 0).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=1)",0.4155128, matrix.getValueAt(1, 1).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=2)",9.463990, matrix.getValueAt(1, 2).asDouble(),  WrappedEngineIntegrationTest.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=3)",1.489836e-12, matrix.getValueAt(1, 3).asDouble(),  WrappedEngineIntegrationTest.EPS);
    }
    ...
    public void testAssignDoubleArray() {
        ...
        Assert.assertEquals(1.0, fromR[0], EPS);
        Assert.assertEquals(2.0, fromR[1], EPS);
        Assert.assertEquals(3.0, fromR[2], EPS);
    }
