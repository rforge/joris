...
public class TestREngineAssign extends AbstractRTestCase {
    ...
    public void testMatrix() {
        ...
        Assert.assertEquals("coeff value is wrong @(row=0,col=0)",-17.579095, matrix.getValueAt(0, 0).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=1)",6.7584402, matrix.getValueAt(0, 1).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=2)",-2.601058, matrix.getValueAt(0, 2).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=0,col=3)",1.231882e-02, matrix.getValueAt(0, 3).asDouble(),  TestWrappedEngine.EPS);

        Assert.assertEquals("coeff value is wrong @(row=1,col=0)",3.932409, matrix.getValueAt(1, 0).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=1)",0.4155128, matrix.getValueAt(1, 1).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=2)",9.463990, matrix.getValueAt(1, 2).asDouble(),  TestWrappedEngine.EPS);
        Assert.assertEquals("coeff value is wrong @(row=1,col=3)",1.489836e-12, matrix.getValueAt(1, 3).asDouble(),  TestWrappedEngine.EPS);
    }
