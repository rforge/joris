...
public class ObjectFormatter extends AbstractObjectFormatter implements
		TypedObjectFormatter<Object> {
	...
	public String format(final Object value) {
		...
		//TODO create instance with constructor (either null arg or this arg)
//		for (final Map.Entry<Class<?>, BaseObjectFormatter<?>> entry : formatters
//				.entrySet()) {
//			if (entry.getKey().isAssignableFrom(objClass)) {
//				// formatters.put(objClass, entry.getValue());
//				@SuppressWarnings("rawtypes")
//				final BaseObjectFormatter rawFormatter = entry.getValue();
//				return rawFormatter.format(value);
//			}
//		}
//		Class<?> superClass = objClass.getSuperclass();
//		while (!java.lang.Object.class.equals(superClass)) {
//			if (formatters.containsKey(superClass)) {
//				@SuppressWarnings("rawtypes")
//				final BaseObjectFormatter rawFormatter = formatters
//						.get(superClass);
//				return rawFormatter.format(value);
//			}
...
//		}
		...
		LOG.warn("no Formatter found for objClass = " + objClass);
		return null;
	}
	...
	private TypedObjectFormatter<?> findFormatterForObject(final Class<?> objClass) {
		if (formatters.containsKey(objClass)) {
			LOG.debug("find formatter for " + objClass);
	...
	private TypedObjectFormatter<?> createFormatter(final Class<?> objClass, final Class<?> formatterClass) {
		...
		for (final Constructor<?> constructor : formatterClass.getConstructors()) {
			...
			else if (constructor.getParameterTypes() != null && constructor.getParameterTypes().length == 1 && ObjectFormatter.class.equals(constructor.getParameterTypes()[0])) {
		...
		if (objectFormatterConstructor != null) {
			try {
				...
				LOG.error("error creating formatter instance", e);
			}
	...
	private Class<?> findFormatterClass(final Class<?> aClassToFormat, final Set<Class<?>> classFilter) {
		...
		while (!java.lang.Object.class.equals(aMatchingClass)
				&& formatterClass == null) {
	...
	private Class<?> findFormatterClassByInterfaces (
			final Class<?> aProxyClassToFormat, final Set<Class<?>> classFilter) {
		final List<Class<?>> interfaces = Arrays.asList(aProxyClassToFormat
				.getInterfaces());
		Class<?> formatterClass = null;
	...
	private Class<?> findFormatterClassFor(final Class<?> anyClass) {
		...
		final StringBuilder classNameBuilder = new StringBuilder(packageName).append(".").append(
						simpleClassName).append("Formatter");
		LOG.debug("try class \"" + classNameBuilder + "\"");
		try {
			return Class.forName(classNameBuilder.toString());
		} catch (final ClassNotFoundException e) {
			LOG.debug("unknown class '" + classNameBuilder + "'");
			return null;
		}
	}
	...
	public static final class ArrayClassArrayWrapper<T> implements
			ArrayWrapper<T> {
		final T value;
	...
	public static final class CollectionClassArrayWrapper<T> extends
			ArrayArrayWrapper<T> implements ArrayWrapper<T> {
		...
		public CollectionClassArrayWrapper(final Class<T> forClass, final Collection<T> collection) {
			...
			LOG.debug("wrapped Collection<"+forClass+">:"+collection);
		}
	}
