...
public class IREXPFormatter extends DelegetableObjectFormatter<IREXP> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(IREXPFormatter.class);
    ...
    public String format(final IREXP obj) {
        ...
        if (convertMethod == null) {
            LOGGER.warn("no formatter available for IREXP.type=" + obj.getType());
            for (Field typeField : IREXP.class.getFields()) {
                if (int.class.equals(typeField.getType())) {
                    try {
                        ...
                        LOGGER.error("format failed for "+obj, e);
                    }
                }
            }
            return null;
