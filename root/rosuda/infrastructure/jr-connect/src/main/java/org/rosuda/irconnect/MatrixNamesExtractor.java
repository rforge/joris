    ...
    public String[] getColNames(final IREXP theMatrix) {
        if (theMatrix.getType() == IREXP.XT_VECTOR) {
            ...
            if (namesVec.size()>1)
            {
                ...
                else throw new IllegalArgumentException("type "+colNames.getType()+" cannot be converted into colnames");
            }
        }
    ...
    public String[] getRowNames(final IREXP theMatrix) {
        if (theMatrix.getType() == IREXP.XT_VECTOR) {
            ...
            if (rowNames.getType()==IREXP.XT_ARRAY_STR) {
                ...
                return null;
            } else throw new IllegalArgumentException("type "+rowNames.getType()+" cannot be converted into rownames");
        }
