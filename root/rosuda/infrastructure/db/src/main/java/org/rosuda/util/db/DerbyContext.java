...
public class DerbyContext extends ProcessContext {
	...
	Process processStartScript() throws IOException {
		...
		final String startScript = MessageFormat.format(derbyStarterProcess,
				classPath);
		log.info("starting database by command:\r\n>" + startScript);
		return createProcessForArg(startScript);
	}

	void processStopScript() throws IOException {
		final String classPath = System.getProperty("java.class.path");
		final String startScript = MessageFormat.format(derbyStopperProcess,
				classPath);
		log.info("starting database by command:\r\n>" + startScript);
	...
	private boolean canCreateConnectionFromDataSource(
