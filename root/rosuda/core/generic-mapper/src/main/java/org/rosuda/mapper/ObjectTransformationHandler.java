...
public class ObjectTransformationHandler<NodeType> {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ObjectTransformationHandler.class.getCanonicalName());
     ...
     * @param parentNode
     * @param traces
     *            needs to be given if a possibilty of loops exists
     */
    public void transform(final Object source, final Node.Builder<NodeType> target, final MappedNodeTrace<NodeType> trace) {
        if (source != null) {
            if (!map(source.getClass(), source, target, trace)) {
                logger.warn("no mapping found for class " + source.getClass() + ", using reflection");
    ...
    private boolean map(final Class<?> mappedClass, final Object source, final Node.Builder<NodeType> node,
            final MappedNodeTrace<NodeType> trace) {
        if (source != null) {
            ...
            if (Proxy.isProxyClass(mappedClass)) {
                ...
                final AbstractGenericMapper<Object, NodeType> genericMapper = (AbstractGenericMapper<Object, NodeType>) mappings
                        .get(mappedClass);
