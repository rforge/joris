...
public abstract class NodeFilter<T> {
	 ...
	 * @return
	 */
	public final boolean accept(final Node.Builder<T> parent, final String... newChildName) {
		boolean accepted = mayCreateChild(parent, newChildName);
		fireEvent(new EventImpl<T>(accepted, parent));
		return accepted;
	}
	protected abstract boolean mayCreateChild(final Node.Builder<T> parent, final String... newChildName);
