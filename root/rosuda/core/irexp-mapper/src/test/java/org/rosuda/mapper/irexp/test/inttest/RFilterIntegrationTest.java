...
public class RFilterIntegrationTest extends AbstractRIntegrationTest {
    ...
    int countAccepted = 0;
    int countRejected = 0;
    private static final Logger logger = LoggerFactory.getLogger(RFilterIntegrationTest.class.getCanonicalName());
    ...
    public void testUnfilteredModel() {
        if (connection == null) {
            logger.warn("no connection available, test cannot be run");
            return;
        }
