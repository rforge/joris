...
public class RFilterIntegrationTest extends AbstractRIntegrationTest {

    int countAccepted = 0;
    int countRejected = 0;
    private static final Logger logger = Logger.getLogger(RFilterIntegrationTest.class.getCanonicalName());
    private ITwoWayConnection connection;
    private ObjectTransformationManager<Object> filterMgr;

    @Before
    public void setUp() throws Exception {
	this.filterMgr = new ObjectTransformationManager<Object>(new NodeBuilderFactory<Object>(), new IREXPMapper<Object>().createInstance());
	...
	final NodeFilter<Object> filterResiduals = new NodeFilter<Object>() {
		...
		"residuals".equals(name) || "effects".equals(name) || "rank".equals(name) || "fitted.values".equals(name) || "assign".equals(name)
			|| "qr".equals(name) || "df.residual".equals(name) || "xlevels".equals(name) || "call".equals(name) || "terms".equals(name)
			|| "model".equals(name)) {
		    return false;
		}
		return true;
		...
		if ("Double".equals(parent.getName()) && parent.getParent() != null) {
		    if (newValue.getType() == Value.Type.STRING
			    ...
			    // "qr".equals(newValue.getString()) ||
				    "aliased".equals(newValue.getString()) || "sigma".equals(newValue.getString()) || "fstatistic".equals(newValue.getString()) || "t value"
					.equals(newValue.getString()))) {
			// manipulate parent node ?
			// dont accept any children and remove this node!
			// parent->getParent():keyValue (container)
			// parent->getParnet()->getParent:map (supercontainer)
