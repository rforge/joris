...
public class ReferenceIntegrationTest {
	...
	public void findOneExtendedLMByConstraintApi() {
		...
		tempOnly.add(new NameVertexConstraint("Temp").addChildConstraint(new NameVertexConstraint("Pr(>|t|)").addValueConstraint(new NumberValueConstraint(1e-4, Relation.LT))));
		...
		tempUnderCoeffOnly.add(new NameVertexConstraint("Coefficients").addChildConstraint(new NameVertexConstraint("matrix").addChildConstraint(new NameVertexConstraint("Temp").addChildConstraint(new NameVertexConstraint("Pr(>|t|)").addValueConstraint(new NumberValueConstraint(1e-4, Relation.LT))))));
		assertEquals(5, graphService.find(tempOnly).size());
		
		final List<VertexConstraint> windConstraint = new ArrayList<VertexConstraint>();
		windConstraint.add(new NameVertexConstraint("Wind").addChildConstraint(new NameVertexConstraint("Pr(>|t|)").addValueConstraint(new NumberValueConstraint(1e-4, Relation.LT))));
		assertEquals(5, graphService.find(windConstraint).size());
		
		windConstraint.add(new NameVertexConstraint("Temp").addChildConstraint(new NameVertexConstraint("Pr(>|t|)").addValueConstraint(new NumberValueConstraint(1e-4, Relation.LT))));
