...
public class LabelledMultiSelectionStep implements MVP<JPanel, LabelledMultiSelectionStep.Model, LabelledMultiSelectionStep.View> {
	...
	private View(final Step step) {
	    ...
	    final String htmlResourceLocation = "/" + View.class.getName().substring(0, View.class.getName().lastIndexOf(".") + 1).replace('.', '/')
		    + localized.get(prefix + ".HTMLDescription");
	    ...
	    getViewContainer().add(new JScrollPane(nodePathField, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED, JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS),
		    BorderLayout.CENTER);
